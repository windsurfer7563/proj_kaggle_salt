...
from models.models import ResNet34, SE_ResNext50_2
import torch
...
def predict(model, config, model_paths,  from_file_names, batch_size, to_path, tta=1, fold_no=0):
    ...
    for model_path in model_paths:
        ...
        with torch.no_grad():
            for batch_num, (inputs, paths) in enumerate(loader):
                ...
                if tta:
                    ...
                    y_pred, image = model.tta_flip(inputs)
                    ...
                    image = torch.sigmoid(logit_image).data.cpu().numpy().astype(float)
        ...
        fold_predictions_img += all_images / models_no


    np.save(str(to_path / (Path('fold_{}.npy'.format(fold_no)))), fold_predictions)
    np.save(str(to_path / (Path('img_fold_{}.npy'.format(fold_no)))), fold_predictions_img)
...
if __name__ == '__main__':
    ...
    arg('--model_type', type=str, default='SE_ResNext50_2', help='network architecture',
        choices=['ResNet34','SE_ResNext50_2'])
